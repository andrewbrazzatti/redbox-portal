...
import {Services as userService} from '../../services/impl/UsersService';
import {Services as rolesService} from '../../services/impl/RolesService';
import {Services as brandingService} from '../../services/impl/BrandingService';
...
export module Controllers {
  ...
  export class UserManagement extends controllers.Core.Controller {
    ...
    public createUser(req, res) {
      ...
      UsersService.addLocalUser(userReq.username, userReq.name, userReq.email, userReq.password).subscribe(userResponse => {
        ...
        if (userReq.roles) {
          ...
          let brand: BrandingModel = BrandingService.getBrand(brandName) || BrandingService.getDefault();
          if (!brand || _.isEmpty(brand.roles)) {
            sails.log.warn(`createUser: branding not resolved for '${brandName}', skipping role assignment`);
    ...
    public updateUser(req, res) {
      ...
      UsersService.updateUserDetails(userReq.id, userReq.name, userReq.email, userReq.password).subscribe(userResponse => {
        ...
        if (userReq.roles) {
          ...
          let brand: BrandingModel = BrandingService.getBrand(brandName) || BrandingService.getDefault();
          if (!brand || _.isEmpty(brand.roles)) {
            sails.log.warn(`updateUser: branding not resolved for '${brandName}', skipping role assignment`);
            let userResponse = new CreateUserAPIResponse();
    ...
    public listSystemRoles(req, res) {
      ...
      let brand: BrandingModel = BrandingService.getBrand(brandName) || BrandingService.getDefault();
      if (!brand || _.isEmpty(brand.roles)) {
        return this.apiFail(req, res, 404, new APIErrorResponse("Branding not found"));
      }
      let response: ListAPIResponse < any > = new ListAPIResponse < any > ();
