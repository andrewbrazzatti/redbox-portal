...
import type { Services as Brandings } from './BrandingService'
// typescript-json-schema is available at runtime from root project's node_modules
...
const globSync: (pattern: string) => string[] =
  typeof globModule.globSync === 'function'
    ...
    ? globModule.sync
    : typeof globModule.default?.globSync === 'function'
    ? globModule.default.globSync
    : typeof globModule.default?.sync === 'function'
    ? globModule.default.sync
    : (pattern: string) => {
        throw new Error('glob sync function not available');
      };
...
export module Services {
  ...
  export class AppConfigService extends services.Core.Service {
    ...
    private getJsonSchema(modelDefinition: any): any {
      ...
      const wildcardPath = `${sails.config.appPath}/packages/redbox-core-types/src/configmodels/*.ts`;
...
declare global {
  var AppConfigService: Services.AppConfigService;
}
