...
export module Services {
  ...
  return firstValueFrom(super.getObservable(UserAudit.create(auditEvent)));
    ...
    public addLocalUser = (username, name, email, password): Observable<UserModel> => {
      ...
      const brandName = defaultBrand?.name || _.get(sails, 'config.auth.defaultBrand', 'default');
      const authConfig = ConfigService.getBrand(brandName, 'auth') || {};
      const usernameField = _.get(authConfig, 'local.usernameField', 'username');
      const passwordField = _.get(authConfig, 'local.passwordField', 'password');
...
declare global {
  var UsersService: Services.Users;
}
