...
declare var module;
import { QueueService, SearchService, SailsSolrConfig, SailsSolrCoreConfig, SailsSolrCoreOptions, BrandingModel, UserModel, RoleModel, RecordModel, SolrDocument, Services as services, RecordsService } from '../../index';
...
class SolrClient {
  ...
  constructor(options: SailsSolrCoreOptions) {
...
class Solr {
  static createClient(options: SailsSolrCoreOptions) {
...
export module Services {
  ...
  export class SolrSearchService extends services.Core.Service implements SearchService {
    ...
    protected async processDynamicImports() {
      const flatModule: any = await import("flat");
      flat = flatModule.default ?? flatModule;
      if (typeof flat === "function" && flat.flatten == null) {
        flat.flatten = flat;
      }
      if (flat.unflatten == null && flatModule.unflatten) {
        flat.unflatten = flatModule.unflatten;
      }
    }

    protected initClient() {
      const solrConfig: SailsSolrConfig = sails.config.solr;
      let coreIds: string[] = Object.keys(solrConfig.cores);
      for (let coreId of coreIds) {
        let solrOpts: SailsSolrCoreOptions = solrConfig.cores[coreId].options;
    ...
    protected async buildSchema() {
      ...
      for (let coreId of coreNameKeys) {
        ...
        const core: SailsSolrCoreConfig = solrConfig.cores[coreId];
    ...
    private getBaseUrl(coreOptions: SailsSolrCoreOptions): string {
...
declare global {
  var SolrSearchService: Services.SolrSearchService;
}
