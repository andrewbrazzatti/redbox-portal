  ...
  export class Raid extends services.Core.Service {
    ...
    */
    ...
    private async mintRaid(oid, record, options, attemptCount:number = 0): Promise<any> {
      ...
      const configuration = new Configuration({ basePath, accessToken: apiToken });
      const api = new RaidApi(configuration);
      const request: Partial<RaidCreateRequest> = {};
      try {
      ...
      try {
        ...
        apiResp = await api.mintRaid(request as RaidCreateRequest);
        _.set(apiResp, 'config.headers.Authorization', '-redacted-');
        const status = apiResp.status;
        const body = apiResp.data;
        sails.log.verbose(JSON.stringify(apiResp.status));
        ...
        if (status == 201) {
          ...
          throw new RBValidationError({
            message: `Failed to mint RAiD for oid ${oid} statusCode ${status} body ${JSON.stringify(body)}`,
            displayErrors: [{code: 'raid-mint-server-error', status: status}],
          });
        }
      } catch (error) {
