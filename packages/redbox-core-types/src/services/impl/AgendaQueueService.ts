...
// Agenda is imported from the root project's node_modules at runtime
// require() is used to avoid TypeScript compilation errors when dependency isn't present
const Agenda = require('agenda').Agenda ?? require('agenda');
...
export module Services {
 ...
 export class AgendaQueue extends services.Core.Service implements QueueService {
      ...
      public async moveCompletedJobsToHistory(job:any) {
        const dbManager = User.getDatastore().manager;
        const collectionName = sails.config.agendaQueue.collection || 'agendaJobs';
...
declare global {
  var AgendaQueueService: Services.AgendaQueue;
}