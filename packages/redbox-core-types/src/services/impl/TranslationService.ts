  ...
  export class Translation extends services.Core.Service {
    ...
    public t(key, context = undefined, langCode: string = 'en', brandingName: string = 'default') {
      const brand = BrandingService.getBrand(brandingName)
        || BrandingService.getBrandById(brandingName)
        || BrandingService.getDefault();
      const brandingId = brand?.id;
      let i18nextInstance = brandingId ? this.i18nextInstances[brandingId] : undefined;

      if (!i18nextInstance) {
        const fallbackInstance = Object.values(this.i18nextInstances)[0] as any;
        if (fallbackInstance) {
          this.logger.warn(`No i18next instance found for brand name: ${brandingName}, using fallback instance`);
          return fallbackInstance.getFixedT(langCode)(key, context);
        }
        this.logger.warn(`No i18next instance found for brand name: ${brandingName}, falling back to key`);
        return key;
      }
...
declare global {
  var TranslationService: Services.Translation;
}
