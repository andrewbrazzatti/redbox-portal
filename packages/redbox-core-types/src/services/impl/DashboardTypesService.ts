...
export module Services {
  ...
  export class DashboardTypes extends services.Core.Service {
    ...
    public async getDashboardTableConfig(brand: BrandingModel, recordType: string, workflowStage: string): Promise<DashboardTableConfig | null> {
      try {
        ...
        if (!recType) {
          ...
          return this.getDashboardTableConfigFromWorkflow(recordType, workflowStage);
        }
    ...
    private getDashboardTableConfigFromWorkflow(recordType: string, workflowStage: string): DashboardTableConfig | null {
      const workflowConfig = _.get(sails.config.workflow, `${recordType}.${workflowStage}`);
      if (!workflowConfig) {
        return null;
      }
      return _.get(workflowConfig, 'config.dashboard.table', {}) as DashboardTableConfig;
    }
...
declare global {
  var DashboardTypesService: Services.DashboardTypes;
}
