...
// Import from the source directly using relative path
import { Services as ReportsModule } from '../../src/services/impl/ReportsService';
...
describe('ReportsService', function () {
  ...
  describe('getDataRows', function () {
    it('should generate valid data rows with Handlebars templates', function (done) {
      ...
      const optTemplateData = {
        brandingAndPortalUrl: "http://localhost:1500/default/rdmp"
      };

      const result = ReportsService.getDataRows(reportConfig, solrData, optTemplateData);
  ...
  describe('getCSVHeaderRow', function () {
    it('should generate valid header row', function (done) {
      ...
      const result = ReportsService.getCSVHeaderRow(reportConfig);
      ...
      expect(result[5], 'Expect sixth element to be Chief Investigator').to.have.equal("Chief Investigator");
