...
type ConfigModelsPrivate = typeof ConfigModels & { modelsMap: Map<string, any> };
// Access private static map for test isolation.
const ConfigModelsAccess = ConfigModels as ConfigModelsPrivate;

describe('ConfigModels', function () {
  let originalModelsMap;

  beforeEach(() => {
    originalModelsMap = new Map(ConfigModelsAccess.modelsMap);
    ConfigModelsAccess.modelsMap = new Map();
  });

  afterEach(() => {
    ConfigModelsAccess.modelsMap = new Map(originalModelsMap);
  });

  it('should get registered config keys', () => {
    ConfigModels.register('systemMessage', {
      modelName: 'SystemMessage',
      title: 'System Messages',
      class: class SystemMessage {}
    });
    ConfigModels.register('authorizedDomainsEmails', {
      modelName: 'AuthorizedDomainsEmails',
      title: 'Authorized Domains and Emails',
      class: class AuthorizedDomainsEmails {}
    });
  ...
  it('should preserve schema metadata when registered', () => {
    const schema = {
      type: 'object',
      properties: {
        items: { type: 'array' }
    ...
    ConfigModels.register('menu', {
      modelName: 'MenuConfig',
      class: class MenuConfig {},
      schema
    });
    ...
    expect(menuInfo.schema).to.deep.equal(schema);
  });
