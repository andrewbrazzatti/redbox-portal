...
module.exports = function redboxCoreLoader(sails) {
  return {
    ...
    configure: function() {
      ...
      // Generate service shims
      const servicesDir = path.resolve(__dirname, '../../services');
      if (!fs.existsSync(servicesDir)) {
        fs.mkdirSync(servicesDir, { recursive: true });
      }
      const serviceStats = generateServiceShims(sails, servicesDir);
      sails.log.verbose(`redbox-core-loader: Generated ${serviceStats.generated} service shims`);

      // Generate controller shims
      const controllersDir = path.resolve(__dirname, '../../controllers');
      const webserviceDir = path.resolve(controllersDir, 'webservice');
      if (!fs.existsSync(controllersDir)) {
        fs.mkdirSync(controllersDir, { recursive: true });
      }
      if (!fs.existsSync(webserviceDir)) {
        fs.mkdirSync(webserviceDir, { recursive: true });
      }
      const controllerStats = generateControllerShims(sails, controllersDir, webserviceDir);
      sails.log.verbose(`redbox-core-loader: Generated ${controllerStats.generated} controller shims`);
    },
