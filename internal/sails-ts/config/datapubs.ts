...
interface DataPubsMetadata {
  ...
  related_works: Array<{ field: string; type: string }>;
  funders: string[];
  subjects: string[];
  DEFAULT_IRI_PREFS: { [key: string]: any };
}

interface DataPubsConfig {
  rootCollection: {
  ...
  sites: { [key: string]: DataPubsSite };
  metadata: DataPubsMetadata;
}

const dataPubsConfig: DataPubsConfig = {
  "rootCollection": {
